...
const FilteredByRegency = () => {
  const [election, setElection] = useState([]);
  const [pending, setPending] = useState(true);
  const [filterOption, setFilterOption] = useState("")
  const [valueRegency, setValueRegency] = useState([])

  const filterRegency = valueRegency.map((item) => item.name);
  ...
  useEffect(() => {
    ValueFiltering.getValueRegency().then((res) => {
      setValueRegency(res.data.data);
    });
  }, []);
    ...
    <>
      <div className="content-election px-2 py-2">
        <div className="table-election text-center">
          ...
          <DataTable
            title={`Perolehan suara 2019 Kabupaten ${filterOption}`}
            columns={columns}
            data={election.filter((row) => {
              if (!filterOption) return row
              return row.kabupaten === filterOption
            })}
...
export default FilteredByRegency;